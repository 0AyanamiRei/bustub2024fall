    ...
    if (const auto *expr_cmpr = dynamic_cast<const ComparisonExpression *>(expr.get()); expr_cmpr != nullptr) {
      switch (expr_cmpr->comp_type_) {
        case ComparisonType::Equal: {
          ...
          if (const auto *_ = dynamic_cast<const ConstantValueExpression *>(expr_cmpr->GetChildAt(1).get());
              _ == nullptr) {
            /**< 右儿子不是常值*/
            constant = 0, colunm = 1;
          }
          if (const auto *colV = dynamic_cast<const ColumnValueExpression *>(expr_cmpr->GetChildAt(colunm).get());
              colV != nullptr) {
      ...
      if (!fail) {
        ...
        if (it != index_vec_.end()) {
          ...
          if (std::adjacent_find(col_id.begin(), col_id.end(), std::not_equal_to<>()) == col_id.end()) {
            ...
            return std::make_shared<IndexScanPlanNode>(seqscan_plan.output_schema_, seqscan_plan.table_oid_, index_oid,
