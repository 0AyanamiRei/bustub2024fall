...
更新测试前, 只支持下面这种index扫描, 且优化器判断的有问题:

```sql
explain (o) select * from t1 where v1 = 1 or v1 = 2 or v1 = 3;
  => IndexScan { index_oid=0, filter=(((#0.0=1)or(#0.0=2))or(#0.0=3)) }
explain (o) select * from t1 where v1 = 1 or v1 = 2 and v2 = 3;
  => SeqScan { table=t1, filter=((#0.0=1)or((#0.0=2)and(#0.1=3))) }
```

更新了测试后, 现在过不了`p3.05-index-scan-btree.slt`测试了, 新加case是`order by`, 然后我发现没有测试到的case是`v1 > 2`这种:

```sql
explain (o,p) select * from t1 order by v1;
=== PLANNER ===
ExternalMergeSort { order_bys=[("Default", "#0.0")] }
  Projection { exprs=["#0.0", "#0.1", "#0.2"] }
    SeqScan { table=t1 }
=== OPTIMIZER ===
IndexScan { index_oid=0 }

explain (o) select * from t1 where v1 > 2;
  => IndexScan { index_oid=0, filter=(#0.0>2) }
explain (o) select * from t1 where v1 > 2 and v1 < 4;
  => IndexScan { index_oid=0, filter=((#0.0>2)and(#0.0<4)) }
```

对于存在B+索引的`order by`语句, 应该直接使用B+树创建的迭代器进行扫描.
